...
function triplicarLista(listaDeNumeros) {
    ...
    for (let i = 0; i < listaDeNumeros.length; i++) {
        ...
        let triple = valorActual * 3;
        listaResultado.push(triple);
    }
...
function encontrarNumeroMayor(listaDeNumeros) {
    ...
    for (let i=1;i<listaDeNumeros.length;i++) {
        ...
        if (valorActual>mayor) {
            mayor=valorActual; // Actualizamos si encontramos uno más grande
        }
    }
    return mayor;
}
...
function ordenarPalabrasPorLongitud(listaDePalabras) {
    ...
    for (let i=0;i<listaDePalabras.length;i++) {
        Lista.push(listaDePalabras[i]);
    }
    Lista.sort(function(palabraA, palabraB) {
...
function encontrarPalabraMasCorta(listaDePalabras){
    ...
    for (let i=1;i<listaDePalabras.length;i++){
        ...
        if (palabraActual.length<masCorta.length){
            masCorta=palabraActual;
        }
    }
    return masCorta;
}
...
function filtrarSoloTruthy(listaDeValores) {
    ...
    for (let i=0;i<listaDeValores.length;i++) {
        let valorActual=listaDeValores[i];
        if (valorActual) {
            listaTruthy.push(valorActual);
        }
    }
    return listaTruthy;
}
...
function enumerarLista(listaDePalabras) {
    ...
    // Unimos los primeros elementos con coma
    for (let i=0;i<listaDePalabras.length-2;i++) {
        resultado=resultado+listaDePalabras[i]+", ";
    }

    // Anteúltimo elemento
    let anteultimo=listaDePalabras[listaDePalabras.length-2];
    resultado=resultado+anteultimo;

    // Último elemento precedido por " y " y finalizado con punto
    let ultimo=listaDePalabras[listaDePalabras.length-1];
    resultado=resultado+" y "+ultimo+".";
